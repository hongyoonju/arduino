...
pinMode(trig_pin,INPUT);//수신을 하기 때문에 OUTPUT으로 설정
}
...
void loop() {
             ...
             //pulsIn함수는 pulsln(핀번호,핀상태)의 형태로 이루어져 있으며 핀 상태가 바뀌면
             //그때까지 경과한 시간을 ms(마이크로 세컨드)단위로 반환시켜줍니다.
             //단위는 마이크로 초입니다. d의 속도는 조당 340미터를 이동하거나, 29마이크로조 당 1센치를 이동합니다 duration= pulseIn(echo_pin, HIGH);
               //초음파
               //초음파 센서에서는 echo핀이 HIGH였다고 초음파를 받으면서 LOW로 바뀌기 때문에
               //pulsIn함수를 이용해서 초음파가 날아온 시간을 측정할 수 있습니다.
               //high였을 떄 시간(초음파가 보냈다 다시 들어온 시간)을 가지고 거리를 계산
